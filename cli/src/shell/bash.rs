...
pub fn hook_script() -> String {
    ...
    [
        ...
        "    if [[ \"$count\" -eq 1 ]]; then\n".to_string(),
        "        READLINE_LINE=$(echo \"$suggestions\" | awk -F'\\t' '{sub(/ +$/, \"\", $1); print $1}')\n".to_string(),
        "        READLINE_POINT=${#READLINE_LINE}\n".to_string(),
        "    else\n".to_string(),
        "        local selected\n".to_string(),
        "        selected=$(echo \"$suggestions\" | fzf --height 40% --reverse --no-sort --cycle --delimiter='\\t' --with-nth=1,2 --nth=1 --tabstop=65 --prompt=\"🧠 HintShell> \" --header=\"Tab/Enter: select\")\n".to_string(),
        "        if [[ -n \"$selected\" ]]; then\n".to_string(),
        "            READLINE_LINE=$(echo \"$selected\" | awk -F'\\t' '{sub(/ +$/, \"\", $1); print $1}')\n".to_string(),
        "            READLINE_POINT=${#READLINE_LINE}\n".to_string(),
        "        fi\n".to_string(),
        "    fi\n".to_string(),
        "}\n".to_string(),
